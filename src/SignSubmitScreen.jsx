...
export default function SignSubmitScreen() {
    ...
    const timeline = [
        ...
        { label: 'Signed forms received', detail: mailedForms ? 'We\'ve received your signed forms at our office.' : 'Awaiting your signed forms.', done: mailedForms || signingProgress.formsReceived },
    ...
    return (
        <div style={{ maxWidth: 700, margin: '0 auto', padding: '32px 24px' }}>
            ...
            <button
                ...
                disabled={mailedForms}
                style={{ ...S.btnPrimary, width: '100%', marginTop: 8, marginBottom: 48, opacity: mailedForms ? 0.6 : 1 }}
            >
                {mailedForms ? '✓ Forms Mailed' : "I've mailed my forms"}
            </button>
                    ...
                    <div key={i} style={{ display: 'flex', gap: 16, marginBottom: 0 }}>
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 32 }}>
                            <div style={{
                                ...
                                background: step.done ? T.success : 'transparent',
                                border: `2px solid ${step.done ? T.success : '#ddd'}`,
                                display: 'flex', alignItems: 'center', justifyContent: 'center',
                            }}>
                                {step.done ? <CheckIcon size={14} color="#fff" /> : <ClockIcon size={14} color="#ccc" />}
                            </div>
                            {i < timeline.length - 1 && <div style={{ width: 2, flex: 1, minHeight: 24, background: step.done ? T.success : '#ddd' }} />}
                        </div>
                        <div style={{ paddingBottom: 20 }}>
                            <p style={{ fontWeight: 600, fontSize: 15, color: step.done ? T.success : T.text }}>{i + 1}. {step.label}</p>
            ...
            <div style={{ marginTop: 24 }}>
                <div style={{ background: T.blueBg, borderRadius: T.radiusSm, padding: '12px 16px', marginBottom: 12 }}>
                    <p style={{ fontSize: 13, color: T.blue, lineHeight: 1.5 }}>Your citizenship certificate is not a travel document. Do not cancel other passports until you have your Canadian passport in hand.</p>
                </div>
                <div style={{ background: T.bgWarm, borderRadius: T.radiusSm, padding: '12px 16px' }}>
                    <p style={{ fontSize: 13, color: T.textSec, lineHeight: 1.5 }}>Processing times are set by IRCC and may change. We will notify you of any updates.</p>
                </div>
            </div>
        </div>
    );
